...
$salaire = $mysqli->real_escape_string($_POST["salaire"]);
$date_embauche = $mysqli->real_escape_string($_POST["date_embauche"]);
...
if (($req = $mysqli->prepare("INSERT INTO `adresse` ( `numero_porte`, `rue`, `ville`, `code_postal`, `province`, `pays`) VALUES (? ,? ,? ,? ,? ,?) ON DUPLICATE KEY UPDATE `id`  = LAST_INSERT_ID(`id`)")) && ($req2 = $mysqli->prepare("INSERT INTO `employes`(`emp_nom`, `salaire`,`date_embauche`, `adresse_id`, `bibli_id`, `emp_user`, `emp_mdp`) VALUES ( ?, ?, ?, LAST_INSERT_ID(),?, ?, ?)"))) {

    $req->bind_param("ssssss", $porte, $rue, $ville, $zipCode, $province, $pays);
    $req2->bind_param("ssssss", $nom, $salaire, $date_embauche, $bibli, $user, $mdp_hashed);
